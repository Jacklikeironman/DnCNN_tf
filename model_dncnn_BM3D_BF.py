...
class DNCNN(object):
    ...
    def test(self):
        ...
        for i in range(len(test_file_list)):
            ...
            MSE_BM3D_DNCNN = np.float32(np.mean(np.square(np.uint8(result*255)/255 - np.uint8(GT_img*255)/255)))
            PSNR_BM3D_DNCNN = np.multiply(10.0, np.log(1.0 * 1.0 / MSE_BM3D_DNCNN) / np.log(10.0))
            print('Picture[%d]   MSE:[%.8f]   PSNR:[%.4f] ---------BM3D+DNCNN' % ((i + 1), MSE_BM3D_DNCNN, PSNR_BM3D_DNCNN))
